...
export default function VerifyPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/verify/${txHash}`)
      ...
      .catch(() => setError("Gagal memverifikasi transaksi"))
      .finally(() => setLoading(false));
  }, [txHash]);

  const formatHash = (h: string) => h && h.length > 18 ? `${h.slice(0, 10)}...${h.slice(-8)}` : h;
  const formatDate = (d: string) => { try { return new Date(d).toLocaleString("id-ID", { dateStyle: "long", timeStyle: "short" }); } catch { return d; } };
  const formatCurrency = (v: number, c: string) => c === "USD" ? `$ ${v?.toLocaleString("en-US")}` : `Rp ${v?.toLocaleString("id-ID")}`;
    ...
    return (
      <div style={{ ...containerStyle, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ textAlign: "center" }}>
          ...
          <div style={{ fontSize: 18, fontWeight: 600, color: "var(--text-secondary, #9ca3af)" }}>Memverifikasi transaksi...</div>
    ...
    return (
      <div style={{ ...containerStyle, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ ...cardStyle, maxWidth: 520, textAlign: "center", padding: "40px 24px" }}>
          ...
          <h1 style={{ fontSize: 22, fontWeight: 800, marginBottom: 8, color: "var(--text-primary, #fff)" }}>Transaksi Tidak Ditemukan</h1>
          <p style={{ color: "var(--text-secondary, #9ca3af)", marginBottom: 20, fontSize: 14 }}>{error || "Hash transaksi tidak valid atau belum terdaftar di blockchain TrustChain."}</p>
    ...
    <div style={containerStyle}>
      <div style={{ maxWidth: 740, margin: "0 auto" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 32, paddingTop: 10 }}>
          ...
          <h1 style={{ fontSize: 13, fontWeight: 700, color: "var(--text-muted, #9ca3af)", textTransform: "uppercase", letterSpacing: "0.14em" }}>Verifikasi Transaksi Blockchain</h1>
        </div>
        ...
        <div style={{
          ...
          <h2 style={{ fontSize: 26, fontWeight: 900, color: "#10b981", marginBottom: 8, letterSpacing: "-0.01em" }}>Terverifikasi di TrustChain</h2>
          <p style={{ color: "var(--text-primary, #ffffff)", fontSize: 15, fontWeight: 600, maxWidth: 600, margin: "0 auto" }}>Transaksi ini tercatat dan tervalidasi secara permanen pada jaringan TrustChain</p>
          <div style={{
            marginTop: 20,
        ...
        <div style={cardStyle}>
          <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 18, display: "flex", alignItems: "center", gap: 10, color: "var(--text-primary, #fff)" }}>
            <span style={{ color: "#10b981", fontSize: 18 }}>◈</span> Detail Transaksi
          </h3>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "16px 24px" }}>
            {[
              ["Tipe", tx.type === "purchase" ? "Pembelian" : tx.type],
              ["Status", tx.status === "confirmed" ? "✓ Dikonfirmasi" : tx.status],
              ["Nilai", formatCurrency(tx.amount, tx.currency)],
              ["Block", `#${tx.blockNumber?.toLocaleString()}`],
              ["Waktu", formatDate(tx.timestamp)],
              ["Mata Uang", tx.currency],
            ].map(([label, value]) => (
          ...
          {tx.description && (
            <div style={{ marginTop: 18, padding: "14px 16px", background: "var(--bg-tertiary, rgba(0,0,0,0.25))", borderRadius: 12, border: "1px solid var(--border-subtle, rgba(255,255,255,0.05))" }}>
              <div style={{ fontSize: 11.5, color: "var(--text-muted, #6b7280)", marginBottom: 6, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em" }}>Deskripsi</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 16 }}>
            <div style={{ padding: "14px 16px", background: "var(--bg-tertiary, rgba(0,0,0,0.25))", borderRadius: 12, border: "1px solid var(--border-subtle, rgba(255,255,255,0.05))" }}>
              <div style={{ fontSize: 11.5, color: "var(--text-muted, #6b7280)", marginBottom: 4, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em" }}>Pengirim</div>
            ...
            <div style={{ padding: "14px 16px", background: "var(--bg-tertiary, rgba(0,0,0,0.25))", borderRadius: 12, border: "1px solid var(--border-subtle, rgba(255,255,255,0.05))" }}>
              <div style={{ fontSize: 11.5, color: "var(--text-muted, #6b7280)", marginBottom: 4, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em" }}>Penerima</div>
        ...
        {data.product && (
          <div style={cardStyle}>
            <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 16, display: "flex", alignItems: "center", gap: 10, color: "var(--text-primary, #fff)" }}>
              <span style={{ color: "#10b981", fontSize: 18 }}>◎</span> Informasi Produk
            </h3>
            <div style={{ fontSize: 18, fontWeight: 800, marginBottom: 4, color: "var(--text-primary, #fff)" }}>{data.product.name}</div>
            <div style={{ fontSize: 13.5, color: "var(--text-muted, #9ca3af)", marginBottom: 14 }}>Kategori: <strong style={{ color: "var(--text-secondary, #e5e7eb)" }}>{data.product.category}</strong></div>
        ...
        {data.umkm && (
          <div style={cardStyle}>
            <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 16, display: "flex", alignItems: "center", gap: 10, color: "var(--text-primary, #fff)" }}>
              <span style={{ color: "#10b981", fontSize: 18 }}>⬡</span> Produsen / UMKM
            </h3>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
              <span style={{
                ...
                {data.umkm.verificationStatus === "verified" ? "✓ Terverifikasi di TrustChain" : "Menunggu Verifikasi"}
              </span>
            </div>
        ...
        {data.supplyChain.length > 0 && (
          <div style={cardStyle}>
            <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 22, display: "flex", alignItems: "center", gap: 10, color: "var(--text-primary, #fff)" }}>
              <span style={{ color: "#10b981", fontSize: 18 }}>◉</span> Riwayat Supply Chain
            </h3>
              ...
              {data.supplyChain.map((event, i) => (
                <div key={event.id} style={{ marginBottom: i < data.supplyChain.length - 1 ? 24 : 0, position: "relative" }}>
                  ...
                  <div style={{ fontSize: 11.5, color: "var(--text-muted, #6b7280)", marginTop: 4 }}>
                    ...
                    {event.updatedBy && <span> · oleh <strong style={{ color: "var(--text-secondary, #d1d5db)" }}>{event.updatedBy}</strong></span>}
                  </div>
        ...
        <div style={{ ...cardStyle, textAlign: "center", padding: "30px 24px" }}>
          <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 14, color: "var(--text-primary, #fff)" }}>Bagikan Verifikasi Ini</h3>
          ...
          <div style={{ fontSize: 12, color: "var(--text-muted, #6b7280)", marginTop: 12, fontWeight: 500 }}>Scan QR code untuk melihat bukti verifikasi on-chain ini</div>
        </div>

        {/* Footer */}
        <div style={{ textAlign: "center", padding: "20px 0 40px", color: "var(--text-muted, #6b7280)", fontSize: 12 }}>
          <p>Diverifikasi oleh <strong style={{ color: "var(--text-primary, #e5e7eb)" }}>TrustChain</strong></p>
