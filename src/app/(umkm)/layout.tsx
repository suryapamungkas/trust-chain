...
export default function UmkmLayout({ children }: { children: React.ReactNode }) {
  ...
  const NAV_ITEMS = [
    { href: "/umkm", label: lang === "id" ? "Dashboard" : "Dashboard", icon: LayoutDashboard },
    { href: "/umkm/marketplace", label: lang === "id" ? "Marketplace" : "Marketplace", icon: ShoppingCart },
  ];

  useEffect(() => {
    if (!loading && (!user || user.role !== "umkm")) {
      router.push("/login");
    }
  }, [user, loading, router]);
  ...
  if (!user || user.role !== "umkm") return null;
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: "var(--bg-primary)" }}>
      ...
      <aside className={`tc-sidebar ${mobileOpen ? "open" : ""}`} style={{
        ...
        <div style={{ padding: "20px 20px 16px", borderBottom: "1px solid var(--border-color)" }}>
          ...
          <div style={{ fontSize: 11, color: "var(--text-muted)", fontWeight: 500 }}>UMKM Panel</div>
        </div>
