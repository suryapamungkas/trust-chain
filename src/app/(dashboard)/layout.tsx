...
import { LayoutDashboard, LogOut, Globe, Menu, X } from "lucide-react";
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const NAV_ITEMS = [
    { href: "/dashboard", label: lang === "id" ? "Dashboard" : "Dashboard", icon: LayoutDashboard },
  ];
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "var(--bg-primary)" }}>
      ...
      {/* Mobile overlay */}
      {mobileOpen && (
        <div className="tc-sidebar-overlay active" onClick={() => setMobileOpen(false)} />
      )}
      ...
      <aside className={`tc-sidebar ${mobileOpen ? "open" : ""}`} style={{
        ...
        <div style={{ padding: "20px 20px 16px", borderBottom: "1px solid var(--border-color)" }}>
          ...
          <div style={{ fontSize: 11, color: "var(--text-muted)", fontWeight: 500 }}>Admin Panel</div>
        </div>
        <nav style={{ flex: 1, padding: "12px 8px" }}>
          {NAV_ITEMS.map(item => (
            <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)} style={{
              display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderRadius: 8,
              color: "var(--text-primary)", background: "var(--bg-hover)", fontSize: 13, fontWeight: 600, textDecoration: "none",
